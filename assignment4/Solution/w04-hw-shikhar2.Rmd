...
```{r}
nutrition <- read.csv(file="nutrition-2018.csv")
nut_model <- lm(Calories ~ Fat + Sugar + Sodium, data=nutrition)
summary(nut_model)
```

**The null and alternative hypotheses-** Null: None of the predictors in the dataset have a significant linear relationship with the response variable - Calories. Alternate: least one of the predictors from the null hypothesis is not 0 or has a significant linear relationship with Calories variable.

**The value of the test statistic-** The value of F statisctic = 6591 which is a large value and it corresponds to a large portion of the variance being explained by the regression.

**The p-value of the test-** The p-value is very small 2.2e-16 indicating we reject the null hypothesis at any reasonable alpha and say that the regression is significant or in other words atleast one of the predictors have a useful linear relationship with Calories variable.

**A statistical decision at $\alpha = 0.01$** The p-value is very small 2.2e-16 indicating we reject the null hypothesis at alpha = 0.01 and say that the regression is significant or in other words atleast one of the predictors have a useful linear relationship with Calories variable.

**A conclusion in the context of the problem** Since the F value is quite high and p-value is quite low we can conclude that the model is useful than the null model and the given set of predictors (Fat, Sugar and Sodium) have a useful linear relationship with Calories variable.
...
**Interpretation** 
Intercept - The mean calorie value for a food with 0 Fat, 0 Sugar and 0 Sodium is 100.45. 
Fat - Mean increase in Calories for 1 gram increase in Fat for a fixed level of Sugar and Sodium = 8.48.
Sugar - Mean increase in Calories for 1 gram increase in Sugar for a fixed level of Fat and Sodium = 3.90
Sodium - Mean increase in Calories for 1 milligram increase in Sodium for a fixed level of Fat and Sugar = 0.0061
...
```{r}
predict(nut_model, newdata = data.frame(Fat=28, Sugar=9,Sodium=950))
```
...
```{r}
summary(nut_model)$sigma
```

**Interpretation** 
The standard deviation (SD) measures the amount of variability, or dispersion from the mean, while the standard error of the mean (SEM) measures how far the sample mean of the data is likely to be from the true population mean. The SEM is always smaller than the SD.
...
**Interpretation** 
we can say that 76.88% of the observed variation in Calories is explained by the linear relationship with the three predictor variables, Fat, Sugar and Sodium.


**(f)** Calculate a 95% confidence interval for $\beta_2$. Give an interpretation of the interval in the context of the problem.

```{r}
confint(nut_model, parm = "Sugar",level = 0.95)
```

**Interpretation**
We are 95% confident that the true change in mean of Calorie for an increase of 1 gm of Sugar and for fixed Fat and Sodium values will be between 3.76 and 4.04.
...
```{r}
confint(nut_model,parm = "(Intercept)",level = 0.99)
```

**Interpretation**
We are 99% confident that the true mean of Calorie for 0 gm of Sugar and 0 gm of Fat and 0 mg of Sodium will be between 96.82 and 104.08.
...
```{r}
predict(nut_model, newdata = data.frame(Fat=24, Sugar=0,Sodium=350), 
        interval = c("confidence"), level = 0.90 )
```

**Interpretation**
The point estimate of calories in large order of McDonald's french fries for this particualr sample is 306.21. We can also say with 90% confidence that the true mean of Calorie count for french fries is between 303.8033 and 308.6224 for a food with 24gm of Fat , 0 gm of Sugar and 350 mg of Sodium.
...
```{r}
predict(nut_model, newdata = data.frame(Fat=21, Sugar=6,Sodium=1200), 
        interval = c("prediction"), level = 0.90 )
```

**Interpretation**
We are 90% confident that the value of calorie content will lie between 176.3678 and  442.4452 for a food with 21 gm of Fat , 6 gm of Sugar and 1200 mg of Sodium.
...
```{r}
goalies <- read.csv(file="goalies.csv")
...
**- The null hypothesis** The predictors - Shots Against, Minutes and Shutouts do not have a significant linear relationship with the response Wins.
**- The value of the test statistic** 462.59
**- The p-value of the test** 2.2e-16
**- A statistical decision at $\alpha = 0.05$** We reject Null Hypothesis since p-value is significantly less than aplha at 0.05.
**- The model you prefer** Model 2 is better than Model 1 is confirmed by the F test which means adding the predictors Shots Against, Minutes and Shutouts to the model 1 makes the model significantly better.
...
**- The null hypothesis** The predictors - Save Percentage, Goals Against Average and Penalties in Minutes do not have a significant linear relationship with the response Wins.
**- The value of the test statistic** 4.052

**- The p-value of the test** 0.007353 

**- A statistical decision at $\alpha = 0.05$** We reject Null Hypothesis since p-value of 0.007 is less than aplha at 0.05.

**- The model you prefer** Model 3 is better than Model 2 is confirmed by the F test which means adding the predictors Save Percentage, Goals Against Average and Penalties in Minutes to the model 2 makes the model better.
...
```{r}
summary(model3)$coefficients["SV",c("Estimate","Pr(>|t|)")]
```

The value of t statistic for model 3 and Saves parameter = -0.06. Since p_value = 0.00013 is significantly less than alpha we can conclude the Saves parameter is a significant contributor to the model.
...
```{r}
y=Ozone$ozone
X=as.matrix(cbind(rep(1, nrow(Ozone)), Ozone[-1]))
...
I would choose Model-3 as the best model since it has the lowest RMSE and uses less predictors than model-1.
...
The sum of the diagonal of C:

```{r}
sum(diag(C))
```

The fifth row of sim_data:
...
Since we know the true sigma, the true distribution of $\hat{\beta}_1$ will be a normal curve with mean of -0.75 and SD of 0.434983
...
As expected the true mean and true SD values of $\hat{\beta}_1$ match the values of mean and SD obtained from simulation. 
...
The curve created with the true known parameters perfectly matches the distribution obtained from the simulation of $\hat{\beta}_1$
...
Since $\hat{\beta}_3$ is not a significant predictor so the p value will be high for most samples and therefore to see values less than 0.1 as significance level will be infrequent hence observing 9.6% cases of beta_3_pval as less than 0.1 appears to be justified.
...
Since $\hat{\beta}_5$ is a significant predictor so the p-value will be less than 0.01 in general and hence to observe 80% cases with p-value less than 0.01 is expected.
