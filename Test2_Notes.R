...
data.frame(x = x, y = y)
data.frame(Petal.Length = 5.10, Species = "versicolor")
pnorm(1, mean, sd, lower.tail=FALSE)
y = exp(2 + 3 * x + 0.35 * x^2 + rnorm(n = sample_size, sd = 3))
airquality = na.omit(airquality)
...
lm(y ~ .      , data = d)
lm(y ~ .      , data = d, subset = keep)
lm(y ~ x1 * x2, data = d)
lm(y ~ x1 * x2 * x3 , data = d)
lm(y ~ x1 + I(x1 ^ 2), data = d)
lm(y ~ x1 + I(x1 ^ 2) + I(x1 ^ 3) + I(x1 ^ 4), data = d)
lm(log(y) ~ x1, data = d)
lm(log(y) ~ x1 + I(x1 ^ 2), data = d)

coef(mod_tooth)["dose"] 
summary(mod_tooth)$coefficients["dose:suppVC", "Pr(>|t|)"]

anova(mod_add, mod_int)[2, "F"]
anova(mod_add, mod_int)[2, "Pr(>F)"] < 0.05

predict(iris_add, new_flower)
confint(iris_add, parm = "Speciesvirginica", level = 0.90)[, "5 %"]
predict(iris_int, newdata , interval = "prediction", level = 0.99)[, "upr"]
exp(predict(fit_log, newdata , interval = "prediction", level = 0.90)[, "upr"])

var(resid(fit_log)[fitted(fit_log) < 3.5]) 
RSS = sum((exp(fitted(fit)) - quiz_data$y) ^ 2)
...
# Calculate the partial correlation coefficient between y and x1 controlling 
# for the effect of the remaining variables.
y_mod  = lm(y ~ . - x1, data = quiz_data)
x1_mod = lm(x1 ~ x2 + x3 + x4 + x5 + x6, data = quiz_data)
cor(resid(y_mod), resid(x1_mod)) # 0.2024066

# Calculate the variance inflation factor of the regression coefficient for x5.
x5_mod = lm(x5 ~ x1 + x2 + x3 + x4 + x6, data = quiz_data)
1 / (1 - summary(x5_mod)$r.squared) # 39.87626

# Fit two additive linear models:
...
ifelse(AIC(full_mod) < AIC(smaller_mod), get_rss(full_mod), get_rss(smaller_mod)) # 39.66296


# Fit two additive linear models:
# One with x1, x2, and x4 as predictors.
# One with x3, x4, x5, and x6 as predictors.
# Report the Adjusted R2 of the model with the better Adjusted R2.
mod_1 = lm(y ~ x1 + x2 + x4, data = quiz_data)
mod_2 = lm(y ~ x3 + x4 + x5 + x6, data = quiz_data)
max(summary(mod_1)$adj, summary(mod_2)$adj) # 0.1390175
