---
title: "Week 1 - Homework Solution"
...
**solution (b)**
The dataset has 83 observations and 11 variables. The dataset consist of sleep times and weights of different mammals including carnivore, omnivore and herbivore. The observations recorded are sleep and awake length in hours and weight of brain and body in Kg.

```{r ex1-solution (b), eval=FALSE}
data(msleep)
class(msleep)
view(msleep)
str(msleep)
...
**solution (a)**
There are 189 observations and 10 variables in the dataset.The observations capture birth weight along with different health conditions like previous premature labours, hypertension, uterine irritability and number of physician visits to examine risk factors associated with Low Infant birth weight.

```{r ex2-solution (a), eval=FALSE}
data("birthwt")
view(birthwt)
?birthwt
str(birthwt)
...
There seems to be a negligible relationship between the two variables as the points are scattered all over the place withought any significant trend. However for mothers between specific weight from 200 to 240 pounds there seems to be some positive association between Birth Weight and Mother's weight but nothing conclusive can be derived unless its studied more.

```{r ex2-solution (b)}
plot(birthwt$lwt, birthwt$bwt,xlab="Mother's weight at last menstrual period (Pounds)",
...
There seems to be no relationship between Birth weight and Mother's weight. The data is spread all over the graph and no trend can be deduced.

```{r ex2-solution (c)}
plot(y=birthwt$bwt, x=birthwt$age,xlab="Mother's age (years)",
     ylab="Birth Weight (grams)",
...
Based on the below boxplot between baby's birth Weight and Mother's smoking status there seems to be slight decrease in baby's weight on average for mothers who smoked. Also the variability specially on the upper end (Baby's weighing more than average) seems to be more for mothers who did not smoke. Also there seems to be an instance of significant underweight baby for mothers who did smoke.

```{r ex2-solution (d)}
boxplot(bwt ~ smoke, data=birthwt,
        xlab="Smoking Status during Pregnancy (0 or 1)",
        ylab="Birth weight (grams)",
...
The histogram seems to be right skewed indicating most foods have less than 400 calories per serving and as calories increase number of foods belonging to those calories seem to go down - hence the right skewness but there seems to be a few foods (more than the usual trend) with significantly high calories (around 900)

```{r ex3-solution (a)}
nutrition <- read.csv("nutrition-2018.csv", header=TRUE)

hist(nutrition$Calories, 
     xlab="Calorie content (kcal)",
     main= "Histogram of calories per serving in different foods",
     col="light blue",
     border = "brown")
```
...
There appears to be no association between Protein and Calories. However the variability of Calories gets signifinantly reduced for foods with protein content greater than 40 grams. Yes for foods with Protein content greater than 40 grams we can predict calorie to be around 400 kcal but for foods with less than 40 grams no such prediction can be made.

```{r ex3-solution (b)}
plot(nutrition$Protein, nutrition$Calories,xlab="Protein content per serving (grams)",
     ylab="Calorie content per serving (kcal)",
     main="Calories Content vs Protein content per serving",
     col = "blue", 
     pch=20,
     cex=0.75)
```
...
The relationship between Calorie content and Calorie Count is approximately linear. Ideally it should be perfectly linear because of 4-4-9 rule. However it would not be perfectly linear because different foods have differing caloric contributions per gram. For example, 1g of fat from one food may yield 8.4 Calories, while 1g of fat from another food may yield 9.37 Calories.

```{r ex3-solution (c)}
Calorie_count = 4*nutrition$Protein+ 4*nutrition$Carbs + 9*nutrition$Fat
plot(Calorie_count, nutrition$Calories,xlab="Calorie Count based on Protein",
     ylab="Calorie content",
     main="Calories content vs Calorie Count based on Protein",
     col = "brown", 
     pch=20,
     cex=0.8)
```
